...
export class UserEntity implements Usuario {
  ...
  @ApiProperty({
    ...
    description: 'Perfil/papel do usuário no sistema. Determina as permissões de acesso.',
    examples: {
      admin: { value: 'admin', description: 'Administrador - acesso total ao sistema' },
      professor: { value: 'professor', description: 'Professor - pode gerenciar suas turmas e frequência' },
      aluno: { value: 'aluno', description: 'Aluno - acesso limitado aos seus dados e notas' }
    }
  })
  role: EnumPerfil;
  ...
  @ApiProperty({
    required: false,
    example: '2025001',
    description: 'Número de matrícula (apenas para alunos). Null para admin e professores.',
    nullable: true
  })
